...
/*DATA PESERTA*/
jqgrid_data.jqGrid({
    url: site_url+'outsourcing/jqgrid_import_data_bond_transaction',
    mtype: "GET",
    datatype: "json",
    postData:{
    ...
    colModel: [
        ...
        ,{label:'Bond Code',name:'bond_code',width:100,formatter:function(cellvalue,rowObj,rowArray){
            var flag_error_bond=rowArray[7];
            if (flag_error_bond=='0') {
        ...
        ,{label:'flag_error_bond',name:'flag_error_bond',hidden:true}
...
$('#btn_edit').click(function(){
    ...
    if (selrow) {
        if (selarrrow.length>1) {
            ...
            bond_code = bond_code.replace('<span style="color:red">','')
            bond_code = bond_code.replace('</span>','')
            $('#import_id','#dialog_edit').val(import_id);
            $('#bond_code_old','#dialog_edit').val(bond_code);
            
            $('#import_data_id','#dialog_edit').val(import_data_id);
            $('#bond_code','#dialog_edit').val(bond_code);
...
btn_back.click(function(e){
    e.preventDefault();
    window.location.href=site_url+'/outsourcing/import_bond_transaction';
})
btn_proses.click(function(e){
    ...
    if (selarrrow.length>0) {
        $.confirm({
            ...
            content:'Proses Import Data Transaksi Bond. Apakah anda yakin?',
            confirmButtonClass:'btn-warning',cancelButtonClass:'btn-danger',
            confirm: function(){
                $.ajax({
                    type:"POST",dataType:"json",data:{
                        imports_data_id:selarrrow
                    },url:site_url+'/outsourcing/do_import_data_bond_transaction',
                    success: function(response){
                        if (response.success==true) {
                            $.alert({
