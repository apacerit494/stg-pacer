...
*/
$("#jqgrid_data").jqGrid({
    url: site_url+'/master/jqgrid_valuation_date',
    mtype: "GET",
    datatype: "json",
    colModel: [
        { label: 'ID', name: 'id', key: true, width: 80, align:'center', hidden:true },
        { label: 'Last Valuation Date', name: 'last_valuation_date', width: 100 },
        { label: 'Current Valuation Date', name: 'current_valuation_date', width: 100 },
        { label: 'Next Valuation Date', name: 'next_valuation_date', align: "left", width: 100 }
...
$('#btn_edit').click(function(e){
    ...
    if (selrow) {
        $.ajax({
            ...
            url:site_url+'/master/get_valuation_date',
            success:function(response){
                $('#jqgrid').hide();
                $('#edit').show();
                $('#id',FormEdit).val(response.id);
                $('#last_valuation_date1',FormEdit).val(Template.ToDatePicker(response.last_valuation_date));
                $('#current_valuation_date1',FormEdit).val(Template.ToDatePicker(response.current_valuation_date));
                $('#next_valuation_date1',FormEdit).val(Template.ToDatePicker(response.next_valuation_date));
        ...
        FormEdit.ajaxSubmit({
            ...
            success: function(response) {
                if(response.success==true){
                    $.alert({
                        title:'Success',icon:'fa fa-check',backgroundDismiss:false,
                        content:'Edit Valuation Date Success.',
                        confirmButtonClass:'btn-success',
                        confirm:function(){
                            $('.alert-error',FormEdit).hide();
                            $('#jqgrid_data').trigger('reloadGrid');
...
$('#btn_delete').click(function(){
    ...
    if (selrow) {
        $.confirm({
            ...
            content:'Delete Valuation Date? Are You Sure?',
            confirmButtonClass:'btn green',
            cancelButtonClass:'btn red',
            confirm:function(){
                $.ajax({
                    type:"POST",dataType:"json",data:{id:selrow},
                    url:site_url+'/master/delete_valuation_date',
                    success:function(response) {
                        if (response.success===true) {
                            $.alert({
                                title:'Success',icon:'fa fa-check',backgroundDismiss:false,
                                content:'Delete Valuation Date Success.',
                                confirmButtonClass:'btn green',
                                confirm:function(){
                                    $('#userfile').val('');
                                    $('#jqgrid_data').trigger('reloadGrid');
...
$('#btn_export').click(function(){
    window.open(site_url+'excel/export_report_valuation_date')
})

$('#btn_pdf').click(function(){
window.open(site_url+'pdf/export_report_valuation_date')
});
})