...
/*DATA PESERTA*/
jqgrid_data.jqGrid({
    url: site_url+'outsourcing/jqgrid_import_data_deposit_transaction',
    mtype: "GET",
    datatype: "json",
    postData:{
    ...
    colModel: [
        ...
        ,{label:'Bank Code',name:'bank_code',width:100,formatter:function(cellvalue,rowObj,rowArray){
            var flag_error_bank_code=rowArray[7];
            if (flag_error_bank_code=='0') {
                return '<span style="color:red">'+cellvalue+'</span>';
            } else {
                return cellvalue;
            }
        }}
        , { label: 'Placement Break Flag', name: 'placement_break_flag', width: 200, align:'center', formatter:function(cellvalue) {
            switch(cellvalue) {
                case "P":
                return 'Placement';
                break;
        ...
        ,{label:'Placement Break Date',name:'placement_break_date',width:100,align:'center',formatter:'date', formatoptions: {srcformat:'Y-m-d',newformat:'d/m/Y'}}
        ,{label:'Nominal;',name:'nominal',width:120,align:'right',formatter:'currency', formatoptions: {decimalSeparator:',',thousandsSeparator:'.',decimalPlaces:0,defaultValue:'0' }}
        ,{label:'flag_error_bank_code',name:'flag_error_bank_code',hidden:true}
...
$('#btn_edit').click(function(){
    ...
    if (selrow) {
        if (selarrrow.length>1) {
            ...
            bank_code = bank_code.replace('<span style="color:red">','')
            bank_code = bank_code.replace('</span>','')
            $('#import_id','#dialog_edit').val(import_id);
            $('#bank_code_old','#dialog_edit').val(bank_code);
            
            $('#import_data_id','#dialog_edit').val(import_data_id);
            $('#bank_code','#dialog_edit').val(bank_code);
...
btn_back.click(function(e){
    e.preventDefault();
    window.location.href=site_url+'/outsourcing/import_deposit_transaction';
})
btn_proses.click(function(e){
    ...
    if (selarrrow.length>0) {
        $.confirm({
            ...
            content:'Proses Import Data Transaksi Deposit. Apakah anda yakin?',
            confirmButtonClass:'btn-warning',cancelButtonClass:'btn-danger',
            confirm: function(){
                $.ajax({
                    type:"POST",dataType:"json",data:{
                        imports_data_id:selarrrow
                    },url:site_url+'/outsourcing/do_import_data_deposit_transaction',
                    success: function(response){
                        if (response.success==true) {
                            $.alert({
                                title:'Proses Transaksi Mutual Fund SELESAI!',icon:'fa fa-check',backgroundDismiss:false,
