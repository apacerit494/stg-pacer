...
/*DATA PESERTA*/
jqgrid_data.jqGrid({
    url: site_url+'outsourcing/jqgrid_import_data_mutual_fund_transaction',
    mtype: "GET",
    datatype: "json",
    postData:{
    ...
    colModel: [
        ...
        ,{label:'Mutual Fund Code',name:'mf_code',width:100,formatter:function(cellvalue,rowObj,rowArray){
            var flag_error_mf=rowArray[7];
            if (flag_error_mf=='0') {
        ...
        ,{label:'flag_error_mf',name:'flag_error_mf',hidden:true}
...
btn_back.click(function(e){
    e.preventDefault();
    window.location.href=site_url+'/outsourcing/import_mutual_fund_transaction';
})
btn_proses.click(function(e){
    ...
    if (selarrrow.length>0) {
        $.confirm({
            ...
            content:'Proses Import Data Transaksi Subscription. Apakah anda yakin?',
            confirmButtonClass:'btn-warning',cancelButtonClass:'btn-danger',
            confirm: function(){
                $.ajax({
                    type:"POST",dataType:"json",data:{
                        imports_data_id:selarrrow
                    },url:site_url+'/outsourcing/do_import_data_mutual_fund_transaction',
                    success: function(response){
                        if (response.success==true) {
                            $.alert({
