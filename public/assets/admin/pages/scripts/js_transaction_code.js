...
*/
$("#jqgrid_data").jqGrid({
    url: site_url+'/master/jqgrid_transaction_code',
    mtype: "GET",
    datatype: "json",
    colModel: [
        { label: 'ID', name: 'id', key: true, width: 80, align:'center', hidden:true },
        { label: 'Bank Code', name: 'bank_code', align: "center", width: 100 },
        { label: 'Bank Name', name: 'bank_name', width: 250 },
        { label: 'Original Transaction Code ', name: 'transaction_code_original', width: 200},
        { label: 'Flag Debit Credit', name: 'debit_credit_flag', width: 200 },
        { label: 'Keterangan', name: 'vat', align:'description', width: 200 }
...
FormAdd.validate({
    ...
    rules: {
        ...
        transaction_code_original:{required:true},
            ...
            success: function(response) {
                if(response.success==true){
                    $.alert({
                        ...
                        content:'Add Transaction Code Success.',
                        confirmButtonClass:'btn-success',
                        confirm:function(){
                            $('.alert-error',FormAdd).hide();
                            $('#jqgrid_data').trigger('reloadGrid');
                            $('#transaction_code_original',FormAdd).val('').select();
                            $('#debit_credit_flag',FormAdd).val('');
                            $('#description',FormAdd).val('');
...
$('#btn_edit').click(function(e){
    ...
    if (selrow) {
        $.ajax({
            ...
            url:site_url+'/master/get_transaction_code',
            success:function(response){
                $('#jqgrid').hide();
                $('#edit').show();
                $('#id',FormEdit).val(response.id);
                $('#bank_code',FormEdit).val(response.bank_code);
                $('#transaction_code_original',FormEdit).val(response.transaction_code_original);
                $('#debit_credit_flag',FormEdit).val(response.debit_credit_flag);
        ...
        FormEdit.ajaxSubmit({
            ...
            success: function(response) {
                if(response.success==true){
                    $.alert({
                        title:'Success',icon:'fa fa-check',backgroundDismiss:false,
                        content:'Edit Transaction Code Success.',
                        confirmButtonClass:'btn-success',
                        confirm:function(){
                            $('.alert-error',FormEdit).hide();
                            $('#jqgrid_data').trigger('reloadGrid');
...
$('#btn_delete').click(function(){
    ...
    if (selrow) {
        $.confirm({
            ...
            content:'Delete Transaction Code? Are You Sure?',
            confirmButtonClass:'btn green',
            cancelButtonClass:'btn red',
            confirm:function(){
                $.ajax({
                    type:"POST",dataType:"json",data:{id:selrow},
                    url:site_url+'/master/delete_transaction_code',
                    success:function(response) {
                        if (response.success===true) {
                            $.alert({
                                title:'Success',icon:'fa fa-check',backgroundDismiss:false,
                                content:'Delete Transaction Code Success.',
                                confirmButtonClass:'btn green',
                                confirm:function(){
                                    $('#userfile').val('');
                                    $('#jqgrid_data').trigger('reloadGrid');
...
$('#btn_export').click(function(){
    window.open(site_url+'excel/export_report_transaction_code')
})

$('#btn_pdf').click(function(){
window.open(site_url+'pdf/export_report_transaction_code')
});
