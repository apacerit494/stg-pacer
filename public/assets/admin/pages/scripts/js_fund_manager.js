...
*/
$("#jqgrid_data").jqGrid({
    url: site_url+'/master/jqgrid_fund_manager',
    mtype: "GET",
    datatype: "json",
    colModel: [
        { label: 'ID', name: 'id', key: true, width: 80, align:'center', hidden:true },
        { label: 'Fund Manager Code', name: 'fm_code', align: "center", width: 150 },
        { label: 'Fund Manager Name', name: 'fm_name', width: 250 },
        { label: 'Address', name: 'address', width: 450 },
        { label: 'Phone', name: 'phone', width: 100 },
        { label: 'Fax', name: 'faximile', width: 100 }
            ...
            success: function(response) {
                if(response.success==true){
                    $.alert({
                        ...
                        content:'Add Fund Manager Success.',
...
$('#btn_edit').click(function(e){
    ...
    if (selrow) {
        $.ajax({
            ...
            url:site_url+'/master/get_fund_manager',
            success:function(response){
                $('#jqgrid').hide();
                $('#edit').show();
                ...
                $('#faximile',FormEdit).val(response.faximile);
            },
            error: function(){
                Template.WarningAlert("Failed to Connect into Databases, Please Contact Your Administrator!");
            }
        ...
        FormEdit.ajaxSubmit({
            ...
            success: function(response) {
                if(response.success==true){
                    $.alert({
                        title:'Success',icon:'fa fa-check',backgroundDismiss:false,
                        content:'Edit Fund Manager Success.',
                        confirmButtonClass:'btn-success',
                        confirm:function(){
                            $('.alert-error',FormEdit).hide();
                            $('#jqgrid_data').trigger('reloadGrid');
...
$('#btn_delete').click(function(){
    ...
    if (selrow) {
        $.confirm({
            ...
            content:'Delete Fund Manager? Are You Sure?',
            confirmButtonClass:'btn green',
            cancelButtonClass:'btn red',
            confirm:function(){
                $.ajax({
                    type:"POST",dataType:"json",data:{id:selrow},
                    url:site_url+'/master/delete_fund_manager',
                    success:function(response) {
                        if (response.success===true) {
                            $.alert({
                                title:'Success',icon:'fa fa-check',backgroundDismiss:false,
                                content:'Delete Fund Manager Success.',
                                confirmButtonClass:'btn green',
                                confirm:function(){
                                    $('#userfile').val('');
                                    $('#jqgrid_data').trigger('reloadGrid');
...
$('#btn_export').click(function(){
    window.open(site_url+'excel/export_report_fund_manager')
})

$('#btn_pdf').click(function(){
window.open(site_url+'pdf/export_report_fund_manager')
});
})