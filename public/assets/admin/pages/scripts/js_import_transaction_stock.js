...
/*DATA PESERTA*/
jqgrid_data.jqGrid({
    url: site_url+'/inhouse/jqgrid_import_transaction_stock',
    mtype: "GET",
    datatype: "json",
    postData: {
    ...
    colModel: [
        ...
        { label: 'Fund Group', name: 'fund_code', width: 100, align:'center'},
        { label: 'Transaction', name: 'sell_buy_flag', width: 100, align:'center',formatter:function(cellvalue){
            switch (cellvalue) {
                case "S":
                return '<span class="label label-sm label-info">Sell</span>';
                break;
                case "B":
                return '<span class="label label-sm label-info">Buy</span>';
                break;
                default:
                return cellvalue;
                break;
        ...
        { label: 'Classification', name: 'classification', width: 100, align:'center',hidden:true},
        { label: 'Broker', name: 'broker_code', width: 100, align:'center',hidden:true},
        { label: 'Trade Date', name: 'trade_date', width: 100, align:'center',formatter:'date', formatoptions: {srcformat:'Y-m-d',newformat:'d/m/Y'}},
        { label: 'Date Time', name: 'imported_stamp', width: 100, align:'center',formatter:'date', formatoptions: {srcformat:'Y-m-d H:i:s',newformat:'d/m/Y H:i:s'}},
        ...
        { label: 'Action', name:"filename", width: 200, align:'center', formatter: function(cellvalue,rowObj,rowArray) {
                ...
                var status=rowArray[8]; //column status
                var process_url = site_url+'inhouse/process_import_data_transaction_stock/'+row_id;
                var download_button = '<a class="btn btn-default btn-sm2" style="margin:5px 0;" href="'+base_url+'assets/admin/files/transaction_stock/'+cellvalue+'" title="'+cellvalue+'"><i class="fa fa-download"></i> &nbsp;Download</a>';
                var process_button = '<a class="btn btn-default btn-sm2" style="margin:5px 0;" href="'+process_url+'" title="Proses Import Transaksi Saham"><i class="fa fa-refresh"></i> &nbsp;Process</a>';
...
$("a#btn-delete").livequery('click',function(){
    ...
    message = "Hapus Data Import Transaksi Saham. Apakah anda yakin ?";
    ...
    $.confirm({
        ...
        confirm:function(){
            $.ajax({
                ...
                dataType:"json",data:{
                    import_id:id
                },url:site_url+"/inhouse/delete_import_data_transaction_stock",
...
form_import.validate({
    ...
    submitHandler: function (form) {
        ...
        if (bValid_tradeDate==false) {
            Template.WarningAlert('Error : Please Check Again!');
        } else {
            ...
            form_import.ajaxSubmit({
                ...
                success: function(response) {
                    ...
                    if(response.success==true){
                        $.alert({
                            title:'Success',icon:'fa fa-check',backgroundDismiss:false,
                            content:'Data Has Been Uploaded and will be Processed Directly.',
                            confirmButtonClass:'btn-success',
                            confirm:function(){
                                alert_error.hide();
                                jqgrid_data.trigger('reloadGrid');
                                ...
                                $('#btn-upload').html('<i class="fa fa-spinner fa-spin"></i> &nbsp;Processing..');
                                $('#btn-upload').attr('disabled',true);
                                var process_url = site_url+'inhouse/process_import_data_transaction_stock/'+response.import_id;
                    ...
                    $('#btn-upload').html('<i class="fa fa-upload"></i> &nbsp;Process');
...
var clearFormImport = function () {
    $('#fund_group_code',form_import).val('');
    $('#sell_buy_flag',form_import).val('');
    $('#classification',form_import).val('');
    $('#broker_code',form_import).val('');
    $('#userfile',form_import).val('');
    progress.hide();
}

$('#trade_date').blur(function(){
    validate_tradedate();
})
$('#trade_date').change(function(){
    validate_tradedate();
})

validate_tradedate = function(){
    tDate = $('#trade_date').val();
    $.ajax({
        type:"POST",dataType:"json",data:{date:tDate},
        url:site_url+'get/validate_holiday',
        async:false,
        success: function(response){
            if (response.valid===true) {
                bValid_tradeDate = true;
                $('#trade_date').removeClass('error');
                $('.has-error-trade_date').html('').hide();
            } else {
                bValid_tradeDate = false;
                $('#trade_date').addClass('error');
                $('.has-error-trade_date').html('<span style="color:red;"><i class="fa fa-warning"></i> Can\'t use holiday</span>').show();
            }
        }
    })
}
