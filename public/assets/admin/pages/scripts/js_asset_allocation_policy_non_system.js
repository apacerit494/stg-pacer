...
*/
$("#jqgrid_data").jqGrid({
    url: site_url+'/master/jqgrid_asset_allocation_policy_non_system',
    mtype: "GET",
    datatype: "json",
    colModel: [
            ...
            success: function(response) {
                if(response.success==true){
                    $.alert({
                        ...
                        content:'Add Asset Alloaction Policy Non System Success.',
...
$('#btn_edit').click(function(e){
    ...
    if (selrow) {
        $.ajax({
            ...
            url:site_url+'/master/get_asset_allocation_policy_non_system',
            success:function(response){
                $('#jqgrid').hide();
                $('#edit').show();
        ...
        FormEdit.ajaxSubmit({
            ...
            success: function(response) {
                if(response.success==true){
                    $.alert({
                        title:'Success',icon:'fa fa-check',backgroundDismiss:false,
                        content:'Edit Asset Allocation Policy Non system Success.',
                        confirmButtonClass:'btn-success',
                        confirm:function(){
                            $('.alert-error',FormEdit).hide();
                            $('#jqgrid_data').trigger('reloadGrid');
...
$('#btn_delete').click(function(){
    ...
    if (selrow) {
        $.confirm({
            ...
            content:'Delete Asset Alloaction Policy Non System? Are You Sure?',
            confirmButtonClass:'btn green',
            cancelButtonClass:'btn red',
            confirm:function(){
                $.ajax({
                    type:"POST",dataType:"json",data:{id:selrow},
                    url:site_url+'/master/delete_asset_allocation_policy_non_system',
                    success:function(response) {
                        if (response.success===true) {
                            $.alert({
                                title:'Success',icon:'fa fa-check',backgroundDismiss:false,
                                content:'Delete Asset Allocation Policy Non System Success.',
                                confirmButtonClass:'btn green',
                                confirm:function(){
                                    $('#userfile').val('');
                                    $('#jqgrid_data').trigger('reloadGrid');
...
$('#btn_export').click(function(){
    window.open(site_url+'excel/export_report_asset_allocation_policy_non_system')
})

$('#btn_pdf').click(function(){
window.open(site_url+'pdf/export_report_asset_allocation_policy_non_system')
});
})