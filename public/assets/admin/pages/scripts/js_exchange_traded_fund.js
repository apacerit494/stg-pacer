...
*/
$("#jqgrid_data").jqGrid({
    url: site_url+'/master/jqgrid_exchange_traded_fund',
    mtype: "GET",
    datatype: "json",
    colModel: [
        { label: 'ID', name: 'id', key: true, width: 80, align:'center', hidden:true },
        { label: 'ETF Code', name: 'etf_code', align: "center", width: 150 },
        { label: 'ETF Name', name: 'etf_name', width: 200 },
        { label: 'ETF Type', name: 'etf_type', width: 150, formatter:function(cellvalue) {
        	switch(cellvalue) {
        		case "E":
        		return 'Eqiuity';
        		break;
        ...
        { label: 'Currency', name: 'currency', width: 100},
        { label: 'Issuer Code', name: 'issuer_code', align: "center", width: 100},
        { label: 'Fund Group Name', name: 'fund_group_name', align: "center", width: 200}
...
FormAdd.validate({
    ...
    rules: {
        ...
        etf_name:{required:true},
        etf_type:{required:true},
        currency:{required:true},
        Issuer_code:{required:true}
            ...
            success: function(response) {
                if(response.success==true){
                    $.alert({
                        ...
                        content:'Add Exchange Traded Fund Success.',
...
$('#btn_edit').click(function(e){
    ...
    if (selrow) {
        $.ajax({
            ...
            url:site_url+'/master/get_exchange_traded_fund',
            success:function(response){
                $('#jqgrid').hide();
                $('#edit').show();
                $('#id',FormEdit).val(response.id);
                $('#etf_code',FormEdit).val(response.etf_code);
                $('#etf_name',FormEdit).val(response.etf_name);
                $('#etf_type',FormEdit).val(response.etf_type);
                $('#currency',FormEdit).val(response.currency);
                $('#fund_group_code',FormEdit).val(response.issuer_code);
            },
            error: function(){
                Template.WarningAlert("Failed to Connect into Databases, Please Contact Your Administrator!");
            }
        ...
        FormEdit.ajaxSubmit({
            ...
            success: function(response) {
                if(response.success==true){
                    $.alert({
                        title:'Success',icon:'fa fa-check',backgroundDismiss:false,
                        content:'Edit Exchange Traded Fund Success.',
                        confirmButtonClass:'btn-success',
                        confirm:function(){
                            $('.alert-error',FormEdit).hide();
                            $('#jqgrid_data').trigger('reloadGrid');
...
$('#btn_delete').click(function(){
    ...
    if (selrow) {
        $.confirm({
            ...
            content:'Delete Exchange Traded Fund? Are You Sure?',
            confirmButtonClass:'btn green',
            cancelButtonClass:'btn red',
            confirm:function(){
                $.ajax({
                    type:"POST",dataType:"json",data:{id:selrow},
                    url:site_url+'/master/delete_exchange_traded_fund',
                    success:function(response) {
                        if (response.success===true) {
                            $.alert({
                                title:'Success',icon:'fa fa-check',backgroundDismiss:false,
                                content:'Delete Exchange Traded Fund Success.',
                                confirmButtonClass:'btn green',
                                confirm:function(){
                                    $('#userfile').val('');
                                    $('#jqgrid_data').trigger('reloadGrid');
...
$('#btn_export').click(function(){
    window.open(site_url+'excel/export_report_exchange_traded_fund')
})

$('#btn_pdf').click(function(){
window.open(site_url+'pdf/export_report_exchange_traded_fund')
});
})