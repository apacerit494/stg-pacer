$(document).ready(function () {
    ...
    $("#jqgrid_data").jqGrid({
        url: site_url + '/CertificationType/jqgrid_certification_type',
        mtype: "GET",
        datatype: "json",
        postData: {
        ...
        colModel: [
            { label: 'ID', name: 'certification_type_id', key: true, width: 100, align: 'center' },
            { label: 'Description', name: 'description', width: 150, search: true, searchoptions: { sopt: ['cn'] } },
            { label: 'Cost', name: 'cost', width: 150, align: 'right', formatter: 'currency', formatoptions: { decimalSeparator: ',', thousandsSeparator: '.', decimalPlaces: 2, defaultValue: '0' } }
        ...
        sortname: "id",
        sortorder: "asc",
        caption: " &nbsp&nbsp&nbsp Certification Type",
        multiselect: false,
        gridview: true,
        // loadonce: true,
    ...
    $('#btn_delete').click(function () {
        ...
        if (selrow) {
            $.confirm({
                ...
                content: 'Delete Certification Type? Are You Sure?',
                ...
                confirm: function () {
                    $.ajax({
                        type: "POST", dataType: "json", data: { certification_type_id: selrow },
                        url: site_url + '/CertificationType/delete_certification_type',
                        success: function (response) {
                            if (response.success === true) {
                                $.alert({
                                    title: 'Success', icon: 'fa fa-check', backgroundDismiss: false,
                                    content: 'Delete Certification Type Success.',
                                    confirmButtonClass: 'btn green',
                                    confirm: function () {
                                        $('#userfile').val('');
                                        $('#jqgrid_data').trigger('reloadGrid');
    ...
    $('#btn_edit').click(function (e) {
        ...
        if (selrow) {
            $.ajax({
                type: "POST", dataType: "json", data: { certification_type_id: selrow },
                url: site_url + '/CertificationType/get_data_certification_type_by_id',
                success: function (response) {
                    $('#jqgrid').hide();
                    $('#edit').show();
                    $('#id', FormEdit).val(response.certification_type_id);
                    $('#description', FormEdit).val(response.description);
                    $('#cost', FormEdit).val(response.cost);
                },
                error: function () {
                    Template.WarningAlert("Failed to Connect into Databases, Please Contact Your Administrator!");
                }
            ...
            FormAdd.ajaxSubmit({
                ...
                success: function (response) {
                    if (response.success == true) {
                        $.alert({
                            title: 'Success', icon: 'fa fa-check', backgroundDismiss: false,
                            content: 'Data Certification Type berhasil ditambahkan',
            ...
            FormEdit.ajaxSubmit({
                ...
                success: function (response) {
                    if (response.success == true) {
                        $.alert({
                            title: 'Success', icon: 'fa fa-check', backgroundDismiss: false,
                            content: 'Edit Certification Type Success.',
                            confirmButtonClass: 'btn-success',
                            confirm: function () {
