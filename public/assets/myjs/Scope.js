$(document).ready(function () {
    ...
    $("#jqgrid_data").jqGrid({
        url: site_url + '/Scope/jqgrid_scope',
        mtype: "GET",
        datatype: "json",
        postData: {
        ...
        colModel: [
            { label: 'ID', name: 'scope_id', key: true, width: 100, align: 'center' },
            { label: 'Scope Code', name: 'scope_code', width: 150, align: 'center', search: true, searchoptions: { sopt: ['cn'] } },
            { label: 'Description', name: 'scope_description', width: 450, align: 'left' }
    ...
    $('#btn_delete').click(function () {
        ...
        if (selrow) {
            $.confirm({
                ...
                content: 'Delete Scope ? Are You Sure?',
                ...
                confirm: function () {
                    $.ajax({
                        ...
                        url: site_url + '/Scope/delete_scope',
                        success: function (response) {
                            if (response.success === true) {
                                $.alert({
                                    title: 'Success', icon: 'fa fa-check', backgroundDismiss: false,
                                    content: 'Delete Scope Success.',
                                    confirmButtonClass: 'btn green',
                                    confirm: function () {
                                        $('#userfile').val('');
                                        $('#jqgrid_data').trigger('reloadGrid');
    ...
    $('#btn_edit').click(function (e) {
        ...
        if (selrow) {
            $.ajax({
                type: "POST", dataType: "json", data: { scope_id: selrow },
                url: site_url + '/Scope/get_data_scope_by_id',
                success: function (response) {
                    $('#jqgrid').hide();
                    $('#edit').show();
                    $('#id', FormEdit).val(response.scope_id);
                    $('#scope_code', FormEdit).val(response.scope_code);
                    $('#scope_description', FormEdit).val(response.scope_description);
                },
                error: function () {
                    Template.WarningAlert("Failed to Connect into Databases, Please Contact Your Administrator!");
                }
